...
Switch to [express-urlrewrite](https://github.com/kapouer/express-urlrewrite) to support rewriting query parameters (e.g.  `/articles?id=1 # → /tasks/1`)
...
* Multiple fields sorting `GET /tasks?_sort=user,views&_order=desc,asc`
...
* Like operator `GET /tasks?title_like=json` (accepts RegExp)
...
* CLI option `-q/--quiet`
* Nested route `POST /tasks/1/comments`
* Not equal operator `GET /tasks?id_ne=1`
...
```bash
# Before
GET /tasks?author=typicode&foo=bar # []
# After
GET /tasks?author=typicode&foo=bar # [{...}, {...}]
```
...
# Support OR
GET /tasks?id=1&id2
GET /tasks?category=javascript&category=html
```
...
GET /tasks?embed=comments
GET /tasks?expand=user
```
